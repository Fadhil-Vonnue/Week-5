...
import { updateWatchList } from "../js/components/updateWatchList";
import { store } from "../main";
import { searchCardMovieAdded } from "../js/components/searchCardMovieAdded";
...
describe("Search Movies", () => {
    ...
    test("test add movies Add", async () => {
        ...
        expect(document.querySelectorAll(`.card[data-id="tt0111161"]`).length).toBe(1);
    });
    test("test add movies delete", async () => {
        ...
        expect(document.querySelectorAll(`.card[data-id="tt0111161"]`).length).toBe(0);
    });
    ...
    test("Add movies to watchlist from search results", async () => {
        const { searchCardMovieAdded: originalFunction } = jest.requireActual("@components/searchCardMovieAdded");
        const but = document.querySelector(".AddButton");
